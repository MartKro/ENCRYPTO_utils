...
void ecc_field::init(seclvl sp, uint8_t* seed) {
	...
	order = new ecc_num(this, bn_order);

	eb_t eb_generator;
	eb_curve_get_gen(eb_generator);
	generator = new ecc_fe(this, eb_generator);

	fe_bytelen = ceil_divide(ECCLVL, 8);
...
void ecc_fe::export_to_bytes(uint8_t* buf) {
	...
	//TODO check if point compression is needed
	eb_write_bin(buf, field->fe_byte_size(), val, false);
}
...
ecc_brickexp::~ecc_brickexp() {
	...
	for(uint32_t i = 0; i < eb_pre_size; ++i) {
		eb_free(eb_table[i]);
	}
}
...
void ecc_brickexp::pow(fe* result, num* e) {
	...
	fe* tmp_fe = new ecc_fe(field, eb_res);
	*result = *tmp_fe;
